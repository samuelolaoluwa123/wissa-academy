...
function StatCard({ icon, label, value, color, isMobile }) {
  return (
    <div style={{ background:'#fff', borderRadius:'14px', border:'1px solid rgba(0,0,0,0.07)', padding: isMobile ? '16px' : '20px', flex:1, minWidth: isMobile ? '45%' : '140px' }}>
  ...
  if (loading) {
    return (
      <DashboardShell role="student">
        <div style={{ padding:'40px', textAlign:'center', color:'#8a94a6', fontSize:'14px' }}>Loading your progress...</div>
      </DashboardShell>
    )
  }
  ...
  return (
    <DashboardShell role="student">
      <div style={{ padding: isMobile ? '20px 16px' : '32px 40px' }}>
        <h1 style={{ fontSize: isMobile ? '20px' : '24px', fontWeight:800, color:'#1a1a2e', margin:'0 0 4px' }}>Progress Report</h1>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'12px', marginBottom:'28px' }}>
          <StatCard icon="⚡" label="Total XP" value={profile.xp_points ?? 0} color="#4a9eff" isMobile={isMobile} />
          <StatCard icon="🔥" label="Day Streak" value={profile.streak_days ?? 0} color="#f5a623" isMobile={isMobile} />
          <StatCard icon="✅" label="Quizzes Passed" value={quizzesPassed} color="#3ee87a" isMobile={isMobile} />
          <StatCard icon="🎓" label="Courses Completed" value={coursesCompleted} color="#8c64ff" isMobile={isMobile} />
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1fr', gap:'20px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
            ...
            {/* Course progress */}
            <div style={{ background:'#fff', borderRadius:'14px', border:'1px solid rgba(0,0,0,0.07)', padding: isMobile ? '18px' : '22px' }}>
              ...
              {enrollments.map(e => (
                <div key={e.id} style={{ marginBottom:'16px' }}>
                  ...
                  <div style={{ height:8, background:'#eef1f6', borderRadius:4, overflow:'hidden' }}>
                    <div style={{ height:'100%', width:`${e.progress_pct ?? 0}%`, background: e.status === 'completed' ? '#3ee87a' : '#4a9eff', borderRadius:4, transition:'width 0.4s' }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background:'#fff', borderRadius:'14px', border:'1px solid rgba(0,0,0,0.07)', padding: isMobile ? '18px' : '22px' }}>
              ...
              {attempts.map(a => (
                <div key={a.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:'10px', paddingBottom:'10px', borderBottom:'1px solid #f0f0f0' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
            ...
            <div style={{ background:'#fff', borderRadius:'14px', border:'1px solid rgba(0,0,0,0.07)', padding: isMobile ? '18px' : '22px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:'12px' }}>
                {badges.map(b => (
                  <div key={b.id} title={b.badge?.description} style={{ textAlign:'center' }}>
