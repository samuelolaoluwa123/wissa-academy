    ...
    <div style={{ minHeight: '100vh', display: 'flex', fontFamily: 'Inter, sans-serif' }}>
      ...
      {!isMobile && (
        <div style={{ flex: '0 0 42%', background: '#0f1b2d', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -80, right: -80, width: 320, height: 320, borderRadius: '50%', background: 'radial-gradient(circle,rgba(74,158,255,0.12),transparent 70%)', pointerEvents: 'none' }} />
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', marginBottom: '40px' }}>
              {[{ val: '4', label: 'Courses' }, { val: '4', label: 'Expert Tutors' }, { val: '100%', label: 'Free' }].map(s => (
                <div key={s.label} style={{ background: 'rgba(255,255,255,0.07)', padding: '14px 22px', borderRadius: '12px', textAlign: 'center' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '32px 20px' : '40px', background: '#eef1f6' }}>
        <div style={{ width: '100%', maxWidth: 420 }}>
          ...
          <div style={{ background: '#fff', borderRadius: '18px', border: '1px solid rgba(0,0,0,0.07)', padding: isMobile ? '24px 20px' : '36px', boxShadow: '0 8px 32px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', background: '#f5f6f8', borderRadius: '10px', padding: '4px', marginBottom: '24px' }}>
              {['signin', 'register'].map(m => (
                <button key={m} onClick={() => { setMode(m); setErrors({}); setMessage('') }} style={{
            ...
            <form onSubmit={handleSubmit}>
              {mode === 'register' && (
                <div style={{ marginBottom: '14px' }}>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="Your full name" style={inp(errors.name)} />
                  {errors.name && <div style={errTxt}>{errors.name}</div>}
                </div>
              )}
              <div style={{ marginBottom: '14px' }}>
                <label style={lbl}>Email Address</label>
                <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="you@email.com" style={inp(errors.email)} />
                {errors.email && <div style={errTxt}>{errors.email}</div>}
              </div>
              <div style={{ marginBottom: mode === 'register' ? '8px' : '18px' }}>
                <label style={lbl}>Password</label>
                <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="••••••••" style={inp(errors.password)} />
              ...
              <button type="submit" disabled={loading} style={{
                ...
                fontWeight: 700, fontSize: '14.5px', marginBottom: '14px',
          ...
          <p style={{ textAlign: 'center', fontSize: '12.5px', color: '#8a94a6', marginTop: '20px' }}>
            ...
            <span onClick={() => { setMode(mode === 'signin' ? 'register' : 'signin'); setErrors({}); setMessage('') }}
